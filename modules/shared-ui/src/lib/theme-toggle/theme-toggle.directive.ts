...
export class ThemeToggleDirective implements OnInit, AfterViewInit {
  ...
  /**
   * Armazena a preferência do tema escuro no localStorage.
   */
  private saveDarkTheme(): void {
    localStorage.setItem(this.THEME_KEY, Theme.Dark);
  }

  /**
   * Remove a preferência do tema escuro no localStorage.
   */
  ...
  /**
   * Determina se o tema escuro deve ser salvo ou removido no localStorage
   * com base no parâmetro `isDarkTheme`.
   *
